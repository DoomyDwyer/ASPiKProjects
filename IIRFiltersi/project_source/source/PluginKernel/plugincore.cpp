...
bool PluginCore::initPluginParameters()
{
	...
	/* filter fc control parameter */ 
	piParam = new PluginParameter(0, "fc", "Hz", controlVariableType::kDouble, 20.000000, 20480.000000, 1000.000000, taper::kVoltOctaveTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.000000);
	piParam->setBoundVariable(&filterFc_Hz, boundVariableType::kDouble);
	addPluginParameter(piParam);

	/* filter Q control parameter */ 
	piParam = new PluginParameter(1, "Q", "", controlVariableType::kDouble, 0.707000, 20.000000, 0.707000, taper::kLinearTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.000000);
	piParam->setBoundVariable(&filterQ, boundVariableType::kDouble);
	addPluginParameter(piParam);

	/* filter Boost/Cut control parameter */ 
	piParam = new PluginParameter(2, "Boost/Cut", "dB", controlVariableType::kDouble, -20.000000, 20.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.000000);
	piParam->setBoundVariable(&boostCut_dB, boundVariableType::kDouble);
	addPluginParameter(piParam);

	/* filter type control parameter */ 
	piParam = new PluginParameter(3, "Filter Type", "LPF1P,LPF1,HPF1,LPF2,HPF2,BPF2,BSF2,ButterLPF2,ButterHPF2,ButterBPF2,ButterBSF2,MMALPF2,MMALPF2B,LowShelf,HiShelf,NCQParaEQ,CQParaEQ,LWRLPF2,LWRHPF2,APF1,APF2,ResonA,ResonB,MAtchLP2A,MatchLP2B,MatchBP2A,MAtchBP2B,ImpInvLP1,ImpInvLP2", "LPF1");
...
void PluginCore::updateParameters()
{
	...
	AudioFilterParameters filterParams = leftAudioFilter.getParameters();
	...
	// --- set on objects
	leftAudioFilter.setParameters(filterParams);
	rightAudioFilter.setParameters(filterParams);
}
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
    ...
    // --- other reset inits
	leftAudioFilter.reset(resetInfo.sampleRate);
	rightAudioFilter.reset(resetInfo.sampleRate);
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	// --- process the audio through the left channel C++ effects object(s)
	double ynL = leftAudioFilter.processAudioSample(xnL);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		double xnR = processFrameInfo.audioInputFrame[1]; //< framework input sample R

		// --- process the audio through the right channel C++ effects object(s)
		double ynR = rightAudioFilter.processAudioSample(xnR);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::boostCut_dB, 0.000000);
